...
@bp.route('/revertir-comando', methods=('POST',))
def revertir_evento_asincrono():
    try:
        #session['uow_metodo'] = 'pulsar'
        evento_dict = request.json
        ...
        evento_dto = map_evento.externo_a_dto(evento_dict)
        id_correlacion = evento_dict.get("id_correlacion", str(uuid.uuid4()))  
        motivo =  evento_dict.get('id_publicacion', '')
        ServicioEvento().revertir_evento(evento_dto.id, motivo, id_correlacion)

        #comando = RevertirEvento(id_correlacion, evento_dto.id, evento_dto.fecha_creacion, evento_dto.fecha_actualizacion, motivo)
        #ejecutar_commando(comando)
